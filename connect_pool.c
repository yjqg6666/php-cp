...
static void cp_add_fail_into_mem(zval *conf, char *data_source);
    ...
    if (zend_hash_find(Z_ARRVAL_P(args), ZEND_STRS("data_source"), (void **) &data_source) == SUCCESS)
    {
        ...
        } else
        {
            ...
            if (EG(exception))
            {
                ...
                cp_add_fail_into_mem(args, Z_STRVAL_PP(data_source));
...
static void cp_add_fail_into_mem(zval *o_arg, char *data_source) {
    zval *args;
    MAKE_STD_ZVAL(args);
    *args = *o_arg;
    zval_copy_ctor(args);
    ...
    {
        zval **zval_source;
        if (zend_hash_find(Z_ARRVAL_P(arr), data_source, strlen(data_source) + 1, (void **) &zval_source) == SUCCESS)
        {//++
            ...
            if (zend_hash_find(Z_ARRVAL_PP(zval_source), ZEND_STRS("count"), (void **) &zval_probably_count) == SUCCESS)
            {
                ...
                zend_hash_del(Z_ARRVAL_P(arr), data_source, strlen(data_source));
                add_assoc_zval(arr, data_source, args);
