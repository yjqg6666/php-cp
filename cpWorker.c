...
//        if (sm_obj->mem == NULL)
//        {
//            if ((sm_obj->mem = cp_mmap_calloc_with_file(sm_obj)) < 0)
//            {
//                cpLog("create posix mmap. Error: %s [%d]", strerror(errno), errno);
//            }
//        }
        if (ret < 0)
        {
...
//                                }
        ...
        {
            if (CPGS->workers_status[i] == CP_WORKER_BUSY)
            {//已经busy了就退出,否则会有跳号bug
                break;
            }
            if (CPGS->workers[i].pid == 0)
            {//争抢的时候就++了 所以会出现0的情况
                continue;
            }
            if (CPGS->workers_status[i] == CP_WORKER_IDLE)
            {//当前worker数大于最小 并且空闲
...
{
    cpShareMemory *sm_obj = &(CPGS->workers[worker_id].sm_obj);
    sprintf(sm_obj->mmap_name, "%s_%d", CP_MMAP_NAME_PRE, 0x3526 + CPGC.port * CP_GROUP_LEN + worker_id);
